...
#        self.execute([self.cmd_halt()])
        ...
        while not (c.data[0] & 0x02):
            self.execute([c])
            self.logger.info("Status: %02x", c.data[0])
...
#        self.freq_cap("enumeration")
    ...
    def cmd_sfr_write(self, addr, data):
        assert addr & 0x80
        return self.cmd_data_write(addr, data)
    ...
    def cmd_data_write(self, addr, data):
        # MOV direct, #data
        return self.cmd_debug_instr(bytes([0x75, addr, data]), False)
    ...
    def cmd_dptr_set(self, addr):
        # MOV DPTR, #data
        return self.cmd_debug_instr(bytes([0x90, addr >> 8, addr & 0xff]), False)
    ...
    def pc(self):
        c = self.cmd_get_pc()
        self.execute([c])
        return int.from_bytes(c.data, "little")

    def halt(self):
        self.execute([self.cmd_halt()])

    def wait_halted(self):
        while not (self.status & self.STATUS_CPU_HALTED):
            self.logger.info("Waiting... %02x %04x", self.status, self.pc)
            pass
...
#        read_back = True
    ...
    def cmd_dptr_a_put(self):
        # MOVX @DPTR, A
        return self.cmd_debug_instr(bytes([0xf0]), False)
...
class DataRead(ComposedOperation):
    ...
    def decompose(self, port):
        return [
            port.cmd_debug_instr(bytes([0xe5, self.address]), False),
            port.cmd_mov_rn_a(0),
            port.cmd_mov_a_rn(0),
            ]
