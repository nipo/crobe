...
class Ltc4266(i2c.Slave):
    def __init__(self, bus, saddr = None):
        i2c.Slave.__init__(self, bus, "LTC4266", saddr)
        self.port_count = 4

    def reg8_read(self, reg):
        rdata = self.write_read(bytes([reg]), 1)
        return rdata[0]
    ...
    def reg16_read(self, reg):
        rdata = self.write_read(bytes([reg]), 2)
        return int.from_bytes(rdata, "little")

    def reg16_write(self, reg, value):
        self.write(bytes([reg, value & 0xff, value >> 8]))
