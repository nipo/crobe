...
class MklI2cTransport(PortComponent):
    ...
    def write(self, data):
        self.logger.info("< %s", binascii.b2a_hex(data))
...
class MklBootloader(PortComponent):
    ...
    def frame_receive(self, tag):
        ...
        if tag == self.FRAME_COMMAND:
            r = hdr + self.port.read(4)
            assert r[0] == self.FRAME_START
            assert r[1] == tag

            length = int.from_bytes(r[2:4], "little")
            crc = r[4:6]
            rsp = self.port.read(length)
        ...
        r = hdr
        assert r[0] == self.FRAME_START
        assert r[1] == tag or r[1] == self.FRAME_START

    def txn_write(self, tag, params, rsp_tag):
        self.logger.info("txn write %02x %s %02x", tag, params, rsp_tag)
        self.frame_send(tag, params)
        time.sleep(.02)
        return self.frame_receive(rsp_tag)

    def txn_read(self, tag):
        self.logger.info("txn read %02x", tag)
        r = self.frame_receive(tag)
    ...
    def ping(self):
        ...
        return self.txn_write(self.FRAME_PING, b'', self.FRAME_PING_RESPONSE)
    ...
    def command(self, cmd, arg = [], data_pkts = []):
        ...
        self.txn_write(self.FRAME_COMMAND, header + data, self.FRAME_ACK)

        r = self.txn_read(self.FRAME_COMMAND)
        hdr = r[:4]
        ...
        for d in data_pkts:
            self.txn_write(self.FRAME_DATA, d, self.FRAME_ACK)
